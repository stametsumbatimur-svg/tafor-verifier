...
def generate_excel_vfinal(df_evaluasi, bulan_nama, tahun, nama_stasiun="UMBU MEHANG KUNDA"):
    # Siapkan buffer memori untuk Streamlit
    output = io.BytesIO()
    
    # Inisialisasi engine xlsxwriter
    writer = pd.ExcelWriter(output, engine='xlsxwriter')
    
    # Kita buat sheet kosong dahulu (data dataframe tidak ditulis langsung)
    df_empty = pd.DataFrame()
    df_empty.to_excel(writer, sheet_name=str(tahun), index=False)
    
    workbook  = writer.book
    worksheet = writer.sheets[str(tahun)]
    
    # ==========================================
    # 1. PENGATURAN FORMAT (STYLING)
    # ==========================================
    format_judul = workbook.add_format({'bold': True, 'align': 'left'})
    format_tengah = workbook.add_format({'align': 'center', 'valign': 'vcenter'})
    format_tengah_bold = workbook.add_format({'bold': True, 'align': 'center', 'valign': 'vcenter', 'border': 1})
    format_border = workbook.add_format({'border': 1, 'align': 'center', 'valign': 'vcenter'})
    
    # ==========================================
    # 2. MENGGAMBAR KOP & ATURAN (BARIS 0 - 15)
    # ==========================================
    # Meniru persis teks dari V FINAL.xlsx
    worksheet.write('A1', 'Instruksi Met./No.029/Verifikasi Prakiraan/I/88', format_judul)
    worksheet.write('A3', 'PERSYARATAN KETELITIAN PRAKIRAAN :', format_judul)
    
    worksheet.write('A5', 'UNSUR METEOROLOGI')
    worksheet.write('C5', 'PERSYARATAN/TOLERANSI KETELITIAN')
    worksheet.write('H5', 'PROSENTASE MINIMUM')
    
    worksheet.write('A8', 'A. Arah angin')
    worksheet.write('C8', '+ 30 Derajat')
    worksheet.write('H8', '80%')
    # ... (Bisa dilanjutkan untuk teks aturan B, C, D, E, F jika diperlukan)
    
    # Header Laporan Bulanan
    # Asumsi baris ke-15 (index 14)
    baris_head = 14
    worksheet.write(baris_head, 0, f'BULAN : {bulan_nama}', format_judul)
    worksheet.write(baris_head, 3, f'TAHUN : {tahun}', format_judul)
    worksheet.write(baris_head, 6, '(SEMUA WAKTU DALAM UTC/GMT)', format_tengah)
    worksheet.write(baris_head, 14, f'STASIUN METEOROLOGI {nama_stasiun}', format_judul)
    
    # ==========================================
    # 3. MENGGAMBAR HEADER TABEL UTAMA
    # ==========================================
    baris_tabel = 16
    worksheet.merge_range(baris_tabel, 0, baris_tabel+1, 0, 'TANGGAL', format_tengah_bold)
    worksheet.merge_range(baris_tabel, 1, baris_tabel, 1, 'JANGKA', format_tengah_bold)
    worksheet.write(baris_tabel+1, 1, 'WAKTU', format_tengah_bold)
    
    # Blok PRAKIRAAN (A - F)
    worksheet.merge_range(baris_tabel, 2, baris_tabel, 7, 'P R A K I R A A N', format_tengah_bold)
    for col_num, huruf in enumerate(['A', 'B', 'C', 'D', 'E', 'F'], start=2):
        worksheet.write(baris_tabel+1, col_num, huruf, format_tengah_bold)
        
    # Blok KENYATAAN (A H B H C H D H E H F H)
    worksheet.merge_range(baris_tabel, 8, baris_tabel, 19, 'K E N Y A T A A N', format_tengah_bold)
    col_idx = 8
    for huruf in ['A', 'B', 'C', 'D', 'E', 'F']:
        worksheet.write(baris_tabel+1, col_idx, huruf, format_tengah_bold)
        worksheet.write(baris_tabel+1, col_idx+1, 'H', format_tengah_bold) # H = Hasil (B/S)
        col_idx += 2

    # ==========================================
    # 4. MENGISI DATA DINAMIS (LOGIKA TANGGAL)
    # ==========================================
    current_row = baris_tabel + 2
    tanggal_terakhir = None
    
    # Loop melalui DataFrame hasil evaluasi dari verif_logic.py
    for index, row in df_evaluasi.iterrows():
        tgl_saat_ini = str(row['Tanggal']).zfill(2) # Misal: '01', '02'
        
        # Logika: Cetak tanggal hanya jika berganti hari
        tampil_tgl = tgl_saat_ini if tgl_saat_ini != tanggal_terakhir else ""
        tanggal_terakhir = tgl_saat_ini
        
        # Tulis Baris (Mapping ke Kolom 0 s/d 19)
        worksheet.write(current_row, 0, tampil_tgl, format_border)
        worksheet.write(current_row, 1, row.get('Jangka_Waktu', '-'), format_border)
        
        # TAF / Prakiraan
        worksheet.write(current_row, 2, row.get('T_Arah', '-'), format_border)
        worksheet.write(current_row, 3, row.get('T_Kec', '-'), format_border)
        worksheet.write(current_row, 4, row.get('T_Vis', '-'), format_border)
        worksheet.write(current_row, 5, row.get('T_Wx', '-'), format_border)
        worksheet.write(current_row, 6, row.get('T_AwanJml', '-'), format_border)
        worksheet.write(current_row, 7, row.get('T_AwanTgi', '-'), format_border)
        
        # METAR & Evaluasi / Kenyataan
        worksheet.write(current_row, 8, row.get('M_Arah', '-'), format_border)
        worksheet.write(current_row, 9, row.get('S_Arah', '-'), format_border)
        worksheet.write(current_row, 10, row.get('M_Kec', '-'), format_border)
        worksheet.write(current_row, 11, row.get('S_Kec', '-'), format_border)
        worksheet.write(current_row, 12, row.get('M_Vis', '-'), format_border)
        worksheet.write(current_row, 13, row.get('S_Vis', '-'), format_border)
        worksheet.write(current_row, 14, row.get('M_Wx', '-'), format_border)
        worksheet.write(current_row, 15, row.get('S_Wx', '-'), format_border)
        worksheet.write(current_row, 16, row.get('M_AwanJml', '-'), format_border)
        worksheet.write(current_row, 17, row.get('S_AwanJml', '-'), format_border)
        worksheet.write(current_row, 18, row.get('M_AwanTgi', '-'), format_border)
        worksheet.write(current_row, 19, row.get('S_AwanTgi', '-'), format_border)
        
        current_row += 1

    # Mengatur lebar kolom agar rapi
    worksheet.set_column(0, 0, 10) # TANGGAL
    worksheet.set_column(1, 1, 12) # JANGKA WAKTU
    worksheet.set_column(2, 19, 8) # Kolom A-F
