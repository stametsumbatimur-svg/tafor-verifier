...
# =========================================================================
# 0. PRA-KOMPILASI REGEX (PERFORMA TINGGI)
# =========================================================================
...
def ekstrak_param_metar_speci(sandi_teks):
    ...
    awan_jml, awan_tgi = "-", "-"
    # 🎯 CAVOK / NSC / NCD di-set tinggi awannya ke 5000 ft agar evaluasi awan BENAR
...
def get_vis_class(vis_value):
    # SOP BMKG 2025: Kelas 4 = 3000-5000m, Kelas 5 = >5000m
...
def proses_verifikasi(df_metar, df_taf, df_speci):
    ...
    for m in metar_list:
        ...
        # 🎯 PERBAIKAN LOGIKA BECMG PERMANEN & PERSISTEN
        for i in range(1, len(parts), 2):
            ...
            if tipe.startswith('FM') or tipe == 'BECMG':
                if target_hour >= s_hr:
                    ...
                    if g_ke != "-": cur_ke = g_ke
                    if g_vi != "-": cur_vi = g_vi
                    if g_wx != "-" and not RE_WX_EXCLUDE.search(isi) and "CAVOK" not in isi: cur_wx = g_wx
                    if g_aj != "-": cur_aj = g_aj
                    if g_at != "-": cur_at = g_at
                    
                    t_ar, t_ke, t_vi, t_wx, t_aj, t_at = cur_ar, cur_ke, cur_vi, cur_wx, cur_aj, cur_at
                    if tipe == 'BECMG':
        ...
        if speci_terkait:
            for sp in speci_terkait:
                sp_ar, sp_ke, sp_vi, sp_wx, sp_aj, sp_at, sp_ts_cb = ekstrak_param_metar_speci(sp[df_speci.columns[1]])
                ...
                baris_speci_final.append({
                    ...
                    'Sandi SPECI':       sp[df_speci.columns[1]], 'TAFOR Berlaku': taf_aktif,
        ...
        baris_analisis_final.append({
            ...
            'M_Arah': m_ar, 'T_Arah': t_ar, 'S_Arah': s_ar,
            'M_Kec': m_ke, 'T_Kec': t_ke, 'S_Kec': s_ke,
            'M_Vis': m_vi, 'T_Vis': t_vi, 'S_Vis': s_vi,
...
def buat_tabel_laporan_excel(df_input):
    ...
    # 🎯 PERBAIKAN: Pengelompokan berdasarkan Tanggal Validitas TAF Utuh (Bukan Tanggal Kalender METAR)
    taf_groups = {}
    for row in df_work:
    ...
    for (tgl_str, taf_sandi), list_rows in taf_groups.items():
        ...
        baris_m_base = list_rows[0][
            1
        ]  # METAR Jam Pertama dari Jendela TAF (misal jam 18:00 UTC)
        ...
        label_perubahan = (
            ...
            if 'AMD' in str(taf_sandi).upper()
                ...
                'Base (COR)' if 'COR' in str(taf_sandi).upper() else 'Base'
            )
        )
        ...
        for i in range(1, len(parts), 2):
            ...
            if tipe.startswith('FM'):
                ...
                prefix = (
                    'T'
                    if 'TEMPO' in tipe
                    else ('B' if 'BECMG' in tipe else 'P')
                )
                jangka_trend = (
            ...
            # 🎯 PERBAIKAN: Mencari METAR tepat pada jam_target di seluruh jendela 12 jam TAF ini
            matched_rows = [
                r for dt_o, r in list_rows if dt_o.hour == jam_target
            ]
