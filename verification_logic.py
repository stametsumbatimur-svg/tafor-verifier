...
def ekstrak_param_metar_speci(sandi_teks):
    ...
    vis = (
        ...
        if 'CAVOK' in sandi_cleaned
            ...
            '-'
            if not (v := RE_VIS.search(sandi_cleaned))
            else v.group(0)
        )
    )
    wx = (
        ...
        if 'CAVOK' in sandi_cleaned
            ...
            '-'
            if not (w := RE_WX.search(sandi_cleaned))
            else w.group(0)
        )
    )
...
def hitung_angin_arah(m_dir, t_dir, m_kec='-', t_kec='-', m_ts_cb=False):
    ...
    if m_str in ['VRB', '///'] and t_str in ['VRB', '///']:
        return (m_str, 'B')
    if (m_str == 'VRB' or t_str == 'VRB') and (m_ts_cb or m_spd < 10 or t_spd < 10):
        return (m_str, 'B')
...
def hitung_cuaca(m_wx, t_wx):
    return 'MATCH', (
        'B'
        if cek_presipitasi_sedang_lebat(m_wx) == cek_presipitasi_sedang_lebat(t_wx)
        else 'S'
    )
...
def proses_verifikasi(df_metar, df_taf, df_speci):
    ...
    for m in metar_list:
        ...
        speci_terkait = [
            ...
            if tgl_jam_aktual
            <= sp['dt_obj']
            <= tgl_jam_aktual + timedelta(minutes=59)
        ]
...
def buat_tabel_laporan_excel(df_input):
    ...
    for (tgl_str, taf_sandi), list_rows in taf_groups.items():
        ...
        for i in range(1, len(parts), 2):
            ...
            # ✅ CIRCULAR TIME WRAP FIX: Pencocokan Waktu METAR Terdekat
            matched_rows = []
