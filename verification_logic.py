...
def proses_verifikasi(df_metar, df_taf, df_speci):
    ...
    for m in metar_list:
        ...
        base_bundle = base_bundle_prev if 'base_bundle_prev' in locals() else (b_ar, b_ke, b_vi, b_wx, b_aj, b_at)
...
def buat_tabel_laporan_excel(df_input):
    ...
    # Memetakan seluruh METAR per jam dari dataset bulanan (day_str, hour_int)
    metar_by_day_hour = {}
    for r in df_work:
        dt = pd.to_datetime(r['Waktu Aktual (UTC)'])
        if dt.minute == 0:
            key = (dt.strftime('%d'), dt.hour)
            if key not in metar_by_day_hour:
                metar_by_day_hour[key] = r
    ...
    for row in df_work:
        ...
        validity_match = re.search(r' (\d{2})\d{2}/\d{2}\d{2} ', str(sandi))
    ...
    for (tgl_str, taf_sandi), list_rows in taf_groups.items():
        ...
        validity_match = re.search(r' (\d{2})(\d{2})/(\d{2})(\d{2}) ', str(taf_sandi))
        if validity_match:
            day_start = validity_match.group(1)
            jam_mulai = validity_match.group(2)
            day_end = validity_match.group(3)
            jam_selesai = validity_match.group(4)
            jangka_base = f'{jam_mulai}-{jam_selesai}'
            jam_akhir_taf = jam_selesai
            jam_base_target = int(jam_mulai)
            tgl_base_target = day_start
        else:
            jam_mulai = '00'
            jam_selesai = '24'
            jangka_base = '00-24'
        ...
        # 🎯 Pencocokan METAR Base secara eksak berdasarkan (Tanggal, Jam)
        baris_m_base = metar_by_day_hour.get((tgl_base_target, jam_base_target))
        if not baris_m_base:
        ...
        for i in range(1, len(parts), 2):
            ...
            if tipe.startswith('FM'):
                ...
                tgl_target = time_match.group(1)
                jam_target = int(time_match.group(2))
                prefix = (
                    'T' if 'TEMPO' in tipe else ('B' if 'BECMG' in tipe else 'P')
                )
                jangka_trend = f'{prefix}.{time_match.group(2)}-{time_match.group(4)}'
            else:
                jam_target = 0
                tgl_target = tgl_str
                jangka_trend = tipe

            # 🎯 Pencocokan METAR Tren secara eksak berdasarkan (Tanggal, Jam)
            baris_m_trend = metar_by_day_hour.get((tgl_target, jam_target))
            if not baris_m_trend:
                baris_m_trend = baris_m_base

            # 🎯 Menyimpan kondisi Base sebelum perubahan BECMG (Opsi A)
            base_bundle_prev = (cur_ar, cur_ke, cur_vi, cur_wx, cur_aj, cur_at)
            ...
            _, s_ar_t, s_ke_t, s_vi_t, s_wx_t, s_aj_t, s_at_t = evaluasi_sandi_tunggal(
                m_obs_trend,
                t_ar, t_ke, t_vi, t_wx, t_aj, t_at,
                base_bundle=base_bundle_prev, # Passing base bundle sebelumnya untuk evaluasi Opsi A
